...
export const loadExistingGraph = async (
  ...
  try {
    ...
    for (const row of await result.getAll()) {
      ...
      // Skip types that are regenerated by global algorithms:
      // - MEMBER_OF / STEP_IN_PROCESS: recomputed by community/process detection
      // - CONTAINS: recomputed by processStructure on all files
      if (type === 'MEMBER_OF' || type === 'STEP_IN_PROCESS' || type === 'CONTAINS') continue;
      relationships.push({
