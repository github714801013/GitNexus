...
describe('MCP Server with project whitelisting', () => {
  ...
  it('should return base repositories and requested env-prefixed indexes when pro is combined with another env', async () => {
    vi.spyOn(backend, 'callTool').mockImplementation(async (method) => {
      if (method === 'list_repos') {
        return [
          { name: 'dev-api', lastCommit: 'abc' },
    ...
    const server = createMCPServer(backend, { envs: ['pro', 'dev'] });
    const result = await callToolViaHandler(server, 'list_repos', {});
    ...
    const repos = JSON.parse(jsonMatch[1]);
    expect(repos.map((repo: any) => repo.name)).toEqual(['dev-api', 'api']);
  });
